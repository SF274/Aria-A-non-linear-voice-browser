...
| T1-02 Audio engine | claude | IN_PROGRESS | HD-10 / DEV-009, out of tier order. Built: `src/content/audio/engine.ts` (singleton context, shared `masterGain -> limiter -> destination` cue bus, `scheduleTone` / `scheduleSpatialTone` / `scheduleSwell`, focal-activity gate, cue log), `mapping.ts` (SPEC 9.3 plus the generative mappings), `transport.ts` (SPEC 9.4 tones, positional and processing ticks, moved off `audio-stubs.ts`, which is deleted). `test/unit/audio.spec.ts` is the test TASKS.md names, against a recording fake `AudioContext`. **Not built:** the F-10 scan (that is T1-03) and SPEC 9.6 rule 4's spoken "Audio cues need a click on the page first." F-10's own criteria are therefore unmet. |
...
| Task | Owner | State | Note |
| --- | --- | --- | --- |
| T2-01 Mutation sonification | claude | DONE | completed 2026-09-19 on human directive HD-10 (out of tier order); `pnpm verify` green (613 Vitest, perf, 56 Playwright). `src/content/audio/mutation.ts`. F-17's criteria checked one by one in `test/e2e/mutation-audio.spec.ts` against real Web Audio: `Search flights` gives 5 point tones at exactly 70 ms spacing, first tone 158–184 ms after the injection (< 250 ms); 20 Hz mutation for 3 s gives 3 bursts, each ≥ 1200 ms apart; a held batch produces silence and the page is audible again once it ends. Generative layers per HD-10 / DEV-009. **Not verified:** nobody has listened to it. |
...
| T2-05 Synthetic text warning (F-22) | claude | DONE | completed 2026-09-20 on human directives HD-13 and HD-14 (out of tier order); `pnpm verify` green (build, typecheck, lint, 686 Vitest, 1 perf, 56 Playwright — exit codes read directly, see E-0xx). `src/sw/gptzero/{client,detect}.ts`, a fourth `<content_authenticity>` prompt part, and a spoken warning composed in code ahead of the answer. Detection runs only on the summary / Q&A path and only adds a sentence: every failure mode returns no verdict and the answer is unchanged, asserted one at a time in `test/unit/gptzero.spec.ts` (36 tests). Two `Settings` fields, service-worker only. See HD-13 / DEV-011. **Not verified:** the thresholds have never been run against real pages with a real key, so the false-positive rate is unknown; and nobody has heard the warning spoken. | **HD-14 extension:** per-paragraph scoring from the same single API call — any paragraph >= 250 chars scoring >= 70% is flagged and promotes an otherwise-clean page to a warning, which is the mostly-human-page-with-one-AI-section case a document score hides. The prompt block names each flagged section by its opening words (sanitized, capped, max 4) so the answer can point at the suspect part. Page text reaches the classifier with paragraph breaks intact (`sanitizeForPromptKeepingBreaks`, opt-in; the answer prompt is byte-identical to before). `pnpm gate:gptzero` + `GPT_ZERO` env var for live verification. 51 tests in `test/unit/gptzero.spec.ts`. See HD-14 / DEV-012. IG-11 PASS 2026-09-20 (human, live key): sample classified 100% A.I. / AI_ONLY; `paragraphs[]` and `sentences[]` both confirmed present with the field names HD-14 reads; a Wikipedia summary produced no warning, so human prose does not trip the rule. **Still unverified:** a multi-entry `paragraphs[]` has not been observed, so tier 1 has not been seen to fire on a real page.

---

## Active work
- **`check` and `uncheck` stop being clicks (human-directed, 2026-09-20; HD-14, DEV-012, NOTES N-019).** A spoken "uncheck" toggled a box the wrong way. The executor was doing what SPEC 7.6.3 step 6 says — guard on the state, then `element.click()` — and a click is relative, so the guard only holds if nothing between it and the click disagrees about the state. `check`/`uncheck` now return success untouched when the state already matches, and otherwise *write* `checked` through the native prototype setter and dispatch bubbling `input` and `change` (the DEV-004 tracker bypass, now shared). A custom `role="switch"` with no `checked` property is still clicked, after reading `aria-checked`. The intent is carried end to end as well: the resolver prompt names the two verbs, the model's `click` on a checkbox is rewritten to the toggle the command named (lone actions only, valid roles only), `ClarificationState` pins the verb across the question, and a toggle word that lands on a link or button falls back to the default verb instead of being refused. `pnpm verify` green; 686 Vitest (+25), 1 perf, 56 Playwright. **No task state changed** — this is a defect fix across T0-13/T0-15's surface, not a new task. **Not verified:** nobody has spoken the command at a real page; two of the six new DOM tests discriminate against the old behaviour, the other four are regression guards (see N-019).
- **Audio engine and generative mutation sonification (human-directed, 2026-09-19; HD-10, DEV-009, NOTES N-018).** The page is now audible as it changes: a swell sized by how much arrived, SPEC 9.8's point tones for each new control, and a flurry of clicks for text churn, all spatialized by where on the page it happened and all synthesized in the browser. T2-01 is `DONE`; T1-02 is `IN_PROGRESS` (the F-10 scan is T1-03, and SPEC 9.6 rule 4's spoken notice is unbuilt). `audio-stubs.ts` is gone — `src/content/audio/{engine,mapping,transport,mutation}.ts` replace it. Two defects were found by the new tests and fixed before they shipped: a burst could spend the 1200 ms rate limit while scheduling nothing audible, and an ambient resume attempted outside a user gesture could permanently silence the extension under SPEC 9.6 rule 4. Unit, DOM, perf and the audio e2e are green (614 Vitest, 1 perf, and `mutation-audio.spec.ts` 3/3); the full `pnpm verify` is **not** consistently green — `qa-exhaustive` D2b, a 60 ms timing tolerance unrelated to audio, failed 2 of 4 full runs and passes 3 of 3 alone. Recorded as E-002, OPEN, with the decisive experiment named and no tolerance widened. Two open items: **E-002** (is D2b a pre-existing flake or new load?) and **HD-11** (a burst can land under the spoken confirmation, which SPEC 9.1 forbids in general terms). **Nobody has heard any of this yet** — every assertion is about what was scheduled.
